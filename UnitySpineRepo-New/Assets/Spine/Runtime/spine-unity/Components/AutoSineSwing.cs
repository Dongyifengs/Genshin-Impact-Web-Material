...
namespace Spine
{
    public class AutoSineSwing : MonoBehaviour
    {
            ...
            if (rootBone != null)
            {
                switch (selfMode)
                {
                    case 1:
                        ...
                        break;
                    case 3:
                        UpdateMode3Func(Time.time, 0.8f);
                        break;
                    case 4:
                        Vector3 scale = this.rootBone.transform.localScale;
                        float dimension = scale.x * scale.y;
                        //Second Param is Amplitude
                        UpdateSpringMagic(rootBone, 10 * Time.deltaTime, 0, 0.8f, dimension < 0 ? -1 : 1);
                        break;
                }
            }
        }
        ...
        {
            if(boneTransform.boneRef == null)
            {
                bool findRef = ManageBoneRef(boneTransform);
            ...
            if (!boneTransform.name.Equals(endBoneName))
            {
                ...
                * selfRotateRange * Mathf.Pow(1 + index * selfAffectByLevel, 1 + selfSpringLevel) + selfRotateCenter;
                ...
                if(0 != selfScaleYRange)
                {
                    ...
                    boneTransform.boneRef.ScaleY = Mathf.Lerp(boneTransform.boneRef.ScaleY, boneTransform.initScaleY + o, mixParam);
                    if(selfSinScaleXSameAsY)
                        boneTransform.boneRef.ScaleX = Mathf.Lerp(boneTransform.boneRef.ScaleX, boneTransform.initScaleX + o, mixParam);
                }
        ...
        * selfScaleXRange * Mathf.Pow(1 + index * selfScaleXAffectByLevel, 1 + selfSpringLevel) + selfScaleXCenter;
                ...
                //rootBone.transform.localScale = new Vector3( boneTransform.boneRef.ScaleX, rootScaleY, 0);
        ...
        void UpdateMode3Func(float time,float param)
        {
            ...
            rootBone.boneRef.WorldX = Mathf.Lerp(rootBone.boneRef.WorldX, rootBone.initX + m, param);
            if (selfMoveYSameAsX)
            {
                ...
                rootBone.boneRef.WorldY = Mathf.Lerp(rootBone.boneRef.WorldY, this.rootBone.initY + m, param);
            }
            else
            {
                m = 0 == selfMoveYCenter ? 0 : UpdateWiggleMode(selfMoveYFreq, selfMoveYAmp, selfMoveYOctaves, time, selfMoveYDelay) + selfMoveYCenter;
                rootBone.boneRef.WorldY = Mathf.Lerp(rootBone.boneRef.WorldY, rootBone.initY + m, param);
            }
                ...
                if (boneTransform.children.Count > 0)
                {
                    for (int f = 0; f < boneTransform.children.Count; f++)
                    {
                        ...
                        if (f == 0)
                        {
                            ...
                            boneTransform.boneRef.Rotation = Mathf.Lerp(boneTransform.boneRef.Rotation, targetRotate, param * boneTransform.autoMoveFriction);
                            boneTransform.boneRef.UpdateWorldTransform();
                        }
